...
public class ShootLongGoUnderTrenchIntakeFromMiddle extends SequentialCommandGroup {
    private static final double UNDER_TRENCH_APPROACH_METERS =
        FieldConstants.Trench.TRENCH_DISTANCE_FROM_ALLIANCE_WALL - Constants.Robot.ROBOT_LENGTH;
    private static final double CENTER_SWEEP_METERS = FieldConstants.Trench.TRENCH_LENGTH;

    public ShootLongGoUnderTrenchIntakeFromMiddle(
        IntakeSubsystem intakeSubsystem,
        CommandSwerveDrivetrain drivetrain,
        TurretSubsystem turretSubsystem
    ) {
        AutoSettings settings = AutoSettings.fromDashboardSelections();
        double trenchEntryTurnDegrees = settings.trenchEntryTurnDegrees();
            ...
            */
            ...
            Commands.deadline(
                Commands.sequence(
                    ...
                    */
                    new DriveRelativeMetersCommand(drivetrain, UNDER_TRENCH_APPROACH_METERS, 0.0),
                    buildTurnIfNeeded(drivetrain, -trenchEntryTurnDegrees),
                    new DriveRelativeMetersCommand(drivetrain, CENTER_SWEEP_METERS, 0.0)
                ),
